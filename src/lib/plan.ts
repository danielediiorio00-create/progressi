...
function parseExerciseLine(text: string): ParsedExercise | undefined {
  ...
  if (m) {
    ...
    // Nessun "3x12": prendo il testo prima di carico/RIR come nome.
    const cut = Math.min(...[weight?.index, rir?.index].filter((i): i is number => i !== undefined), line.length)
    name = line.slice(0, cut)
  }
...
/** "Leg press 3×12 · 60 kg · RIR 2" oppure "Plank a tempo 3×45 s". */
